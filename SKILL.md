...
**v2.1 (2026-06-01)** — Setup step 8 **baseline check** (git freshness before analysis; prevents the wasted-Analyst-round-on-stale-branch failure) and Stage 0 **glossary resolve** (confirm transcribed/voice domain terms absent from code before Stage 1). The workspace overlay: a product component taxonomy (attachment=chat vs screen-level) in Stage 1, and expanded §Ticket quality (no-redundancy / attach-not-describe / no-meta / verify-absence / «не делаем» only-for-traps).
